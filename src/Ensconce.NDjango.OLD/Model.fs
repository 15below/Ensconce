...
module Model =
    ...
    ///
    [<ParserNodes.Description("Provides the name of the model class to be used with the template")>]
    type internal Tag() =
        interface ITag with
            ...
            member this.Perform token context tokens =
                ...
                let rec parse_args (args: Lexer.TextToken list) =
                    match args with
                    | arg :: remainder ->
                ...
                (
                    {new ParserNodes.TagNode(context, token, this) with
                        override x.elements =
                            model
                            |> List.map (fun item -> ParserNodes.TypeNameNode(context, Lexer.Text (snd item)) :> INode)
