...
namespace Ensconce.NDjango.Tests
{
    public partial class TestsRunner
    {
        ...
        public void model_tag_designer_with_variable()
        {
            new TestDescriptor(
                ...
                "{% model Model:Ensconce.NDjango.Tests.TestModel %}{{ Model }}",
                new[]
                    {
                            StandardNode(0, 61),
                            StandardNode(3, 5),
        ...
        public void model_tag_designer()
        {
            new TestDescriptor(
                "model-tag-designer",
                "{% model Model:Ensconce.NDjango.Tests.TestModel %}{{ }}",
                new[]
                    {
                        StandardNode(0, 55),
                        StandardNode(3, 5),
                        ErrorNode(52, 1, EmptyList, 2, "Could not parse some characters | "),
                        VariableNode(52, 1, 2,
                                     "Variables and attributes may not be empty, begin with underscores or minus (-) signs: ' '",
                                     "Standard", "ToString", "GetHashCode", "GetType", "Model", "MethodString",
                                     "MethodInt", "ToString",
        ...
        public void model_tag_designer_multiple()
        {
            new TestDescriptor(
                ...
                "{% model Model1:Ensconce.NDjango.Tests.TestModel %}{% model Model2:Ensconce.NDjango.Tests.TestModel2 %}{{ }}",
                new[]
                    {
                        StandardNode(0, 108),
                        StandardNode(3, 5),
                        StandardNode(54, 5),
                        ErrorNode(105, 1, EmptyList, 2, "Could not parse some characters | "),
                        VariableNode(105, 1, 2,
                                     "Variables and attributes may not be empty, begin with underscores or minus (-) signs: ' '",
        ...
        {
            return new DesignerData(position, length, Enumerable.ToArray<string>(standardFilters()), errorSeverity, errorMessage);
        }

        private DesignerData StandardNode(int position, int length, int errorSeverity, string errorMessage)
        {
            return new DesignerData(position, length, Enumerable.ToArray<string>(standardTags()), errorSeverity, errorMessage);
        }

        private DesignerData StandardFilter(int position, int length)
        {
            return new DesignerData(position, length, Enumerable.ToArray<string>(standardFilters()), -1, String.Empty);
        }

        private DesignerData StandardNode(int position, int length)
        {
            return new DesignerData(position, length, Enumerable.ToArray<string>(standardTags()), -1, String.Empty);
        }
    ...
    public class TestTyperesolver : TypeResolver.ITypeResolver
    {
