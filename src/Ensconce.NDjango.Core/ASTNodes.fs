...
module internal ASTNodes =
    ...
    let get_template (manager:ITemplateManager) resolver model (templateRef:Expressions.FilterExpression) context =
    ...
    type TemplateNameExpression(context:IParsingContext, expression: Lexer.TextToken) =
        inherit Expressions.FilterExpression (context, expression)
    ...
    type SuperBlockPointer = {super:ParserNodes.TagNode}
    ...
    and SuperBlock (context, token, tag, parents: BlockNode list) =
        inherit ParserNodes.TagNode(context, token, tag)
    ...
    and BlockNode(parsing_context, token, tag, name: Lexer.TextToken, nodelist: INodeImpl list) =
        inherit ParserNodes.TagNode(parsing_context, token, tag)
        ...
        override x.elements = BlockNameNode(parsing_context, Lexer.Text name) :> INode :: base.elements
    ...
    and BlockNameNode (context, token) =
        inherit ParserNodes.ValueListNode
            (
                context,
                NodeType.BlockName,
                token,
        ...
        let rec unfold_nodes (nodes: INode seq) =
            ...
            Seq.choose
                (fun node ->
                    match node with
                    | :? ParserNodes.Node | :? ParserNodes.ParsingContextNode ->
                            ...
                            | :? TemplateNameExpression as _base ->
                                ...
                                |> Seq.choose
                                    (function
                                        ...
                                        | :? ParserNodes.ParsingContextNode as context_node -> Some (Context (context_node :> INode).Context)
                                        | _ as node -> None
                                    )
                            | _ -> Seq.empty
