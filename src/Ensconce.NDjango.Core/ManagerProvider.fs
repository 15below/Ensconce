...
module Defaults =
    ...
    let internal standardFilters =
        new Map<string, ISimpleFilter>([])
            ...
            ++ ("escape", (new Filters.EscapeFilter() :> ISimpleFilter))
            ++ ("force_escape", (new Filters.ForceEscapeFilter() :> ISimpleFilter))
            ++ ("slugify", (new Filters.Slugify() :> ISimpleFilter))
            ++ ("truncatewords" , (new Filters.TruncateWords() :> ISimpleFilter))
            ++ ("urlencode", (new Filters.UrlEncodeFilter() :> ISimpleFilter))
            ++ ("urlize", (new Filters.UrlizeFilter() :> ISimpleFilter))
            ++ ("urlizetrunc", (new Filters.UrlizeTruncFilter() :> ISimpleFilter))
            ++ ("wordwrap", (new Filters.WordWrapFilter() :> ISimpleFilter))
            ++ ("default_if_none", (new Filters.DefaultIfNoneFilter() :> ISimpleFilter))
            ++ ("linebreaks", (new Filters.LineBreaksFilter() :> ISimpleFilter))
            ++ ("linebreaksbr", (new Filters.LineBreaksBrFilter() :> ISimpleFilter))
            ++ ("striptags", (new Filters.StripTagsFilter() :> ISimpleFilter))
            ++ ("join", (new Filters.JoinFilter() :> ISimpleFilter))
            ++ ("yesno", (new Filters.YesNoFilter() :> ISimpleFilter))
            ++ ("dictsort", (new Filters.DictSortFilter() :> ISimpleFilter))
            ++ ("dictsortreversed", (new Filters.DictSortReversedFilter() :> ISimpleFilter))
            ...
            ++ ("pluralize", (new Filters.PluralizeFilter() :> ISimpleFilter))
    ...
    let internal standardTags =
        new Map<string, ITag>([])
            ...
            ++ ("comment", (new Misc.CommentTag() :> ITag))
            ++ ("cycle", (new Cycle.Tag() :> ITag))
            ++ ("debug", (new Misc.DebugTag() :> ITag))
            ++ ("extends", (new LoaderTags.ExtendsTag() :> ITag))
            ++ ("filter", (new FilterTag.Tag() :> ITag))
            ++ ("firstof", (new Misc.FirstOfTag() :> ITag))
            ...
            ++ ("regroup", (new Misc.RegroupTag() :> ITag))
            ++ ("spaceless", (new Misc.SpacelessTag() :> ITag))
            ++ ("ssi", (new LoaderTags.SsiTag() :> ITag))
            ++ ("templatetag", (new Misc.TemplateTag() :> ITag))
            ++ ("widthratio", (new Misc.WidthRatioTag() :> ITag))
            ++ ("with", (new Misc.WithTag() :> ITag))
...
type TemplateManagerProvider (settings:Map<string,obj>, tags, filters, loader:ITemplateLoader, dictionary) =
    ...
    let generate_error_node_for_tag (ex: System.Exception) token context tokens =
        ...
        | (Some blockToken, (:? SyntaxError as syntax_error)) ->
            ...
            then
                raise (SyntaxException(syntax_error.Message, Lexer.Block blockToken))
            else
                Some ( ({
                            new ParserNodes.ErrorNode(context, Lexer.Block blockToken, new Error(2, syntax_error.Message))
                                with
                                    /// Add TagName node to the list of elements
                                    override x.elements =
                                        (new ParserNodes.TagNameNode(context, Lexer.Text blockToken.Verb) :> INode)
    ...
    let parse_token (context:IParsingContext) tokens token =
        ...
        | Lexer.Text textToken ->
            ({new ParserNodes.Node(context, token)
                with
        ...
        | Lexer.Variable var ->
            ...
            let expression = new Expressions.FilterExpression(context, var.Expression)
        ...
        | Lexer.Comment comment ->
            ...
            ({new ParserNodes.Node(context, token) with override x.node_type = NodeType.Comment} :> INodeImpl), context, tokens

        | Lexer.Error error ->
            if (settings.[Constants.EXCEPTION_IF_ERROR] :?> bool)
                then raise (SyntaxException(error.ErrorMessage, Lexer.Token.Error error))
            ({
                        new ParserNodes.ErrorNode(context, token, new Error(2, error.ErrorMessage))
                            with
                                override x.elements =
                                    ...
                                    | "{%" ->
                                        ...
                                        (new ParserNodes.TagNameNode(context, Lexer.Text name_token )
                                                :> INode) :: base.elements
                                    | _ -> base.elements
                        } :> INodeImpl), context, tokens
    ...
    let terminals (block : Lexer.BlockToken) =
    ...
    let rec parse_internal (context:IParsingContext) nodes tokens =
       ...
       | LazyList.Cons(token, tokens) ->
            ...
            | Lexer.Block block when context.TagClosures |> List.exists (terminals block).Equals ->
                 (context, (new ParserNodes.CloseTagNode(context, block) :> INodeImpl) :: nodes, tokens)
            | _ ->
    ...
    let rec seek_internal (context:IParsingContext) length tokens =
        ...
        | LazyList.Cons(token, tokens) ->
            ...
            | Lexer.Block block when context.TagClosures |> List.exists block.Verb.Value.Equals ->
                 length, Some (ParserNodes.CloseTagNode(context, block) :> INodeImpl), tokens
            | _ ->
    ...
    interface ITemplateManagerProvider with
        ...
        member x.LoadTemplate ((name, resolver, model)) =
            lock lockProvider
                (fun() ->
                    let t = ((new Template.Impl((x :> ITemplateManagerProvider), loader.GetTemplate(name), resolver, model) :> ITemplate), System.DateTime.Now)
    ...
    interface IParser with
        ...
        member x.Parse parent tokens context =
            ...
            | hd::_ ->
                ...
                ((new ParserNodes.ParsingContextNode(context, start_pos, end_pos - start_pos) :> INodeImpl) :: result, tokens)

        /// Parses the template From the source in the reader into the node list
        member x.ParseTemplate template =
            (x :> IParser).ParseTemplate (template, new TypeResolver.DefaultTypeResolver() :> TypeResolver.ITypeResolver, TypeResolver.ModelDescriptor(Seq.empty))
        ...
        member x.ParseTemplate (template, resolver, model) =
            ...
            (x :> IParser).Parse None (Lexer.tokenize template) (ParsingContext.Implementation(x, resolver, model)) |> fst
