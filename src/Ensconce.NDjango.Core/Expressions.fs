...
module Expressions =
    ...
    type private Filter(context, expression_token:Lexer.TextToken, filter_match:Match) =
        let filter_token = expression_token.CreateToken(filter_match)
        let filter_name = filter_match.Groups.["filter_name"]
        let name_node =
            new ParserNodes.FilterNameNode (
                context,
        ...
        let args = filter_match.Groups.["arg"].Captures |> Seq.cast |> Seq.toList
                |> List.map
                    (fun (c) ->
                        new Variables.Variable(context, expression_token.CreateToken(c))
                    )
        ...
        let error, filter =
            ...
            | :? SyntaxError as ex ->
                ...
                then
                    raise (SyntaxException(ex.Message, Lexer.Text filter_token))
                else
        ...
        member x.Perform (context, input) =
            match filter with
            | None -> raise (SyntaxException(error.Message, Lexer.Text filter_token))
            | Some f ->
                match f with
                | :? IFilter as std ->
                    let param =
    ...
    type FilterExpression (context:IParsingContext, expression: Lexer.TextToken) =
            ...
            matches |> Seq.cast |> Seq.fold
                ...
                fun (offset, error, variable, filters) (mtch:Match) ->
                        ...
                        then
                            ...
                            | None ->
                                ...
                                if var_match.Success then
                                    offset+mtch.Length, error, Some (new Variables.Variable(context, expression.CreateToken var_match)), filters
                                else
                    ...
                    | :? SyntaxError as ex ->
                        if (context.Provider.Settings.[Constants.EXCEPTION_IF_ERROR] :?> bool)
                        then
                            raise (SyntaxException(ex.Message, Lexer.Text expression))
                        else
        ...
        let variable =
            match variable with
            | Some v -> v
            | None -> new Variables.Variable(context, expression)
        ...
        let error =
            ...
            then error
                ...
                if (context.Provider.Settings.[Constants.EXCEPTION_IF_ERROR] :?> bool)
                then
                    raise (SyntaxException(message, Lexer.Text expression))
                else
                    new Error(2, message)
        ...
        member this.ResolveForOutput manager walker =
            ...
            | Some o ->
                ...
                | _ as v ->
                    match if needsEscape then Utilities.escape v else string v with
                    | "" -> None
    ...
    type ExpressionType(expression: FilterExpression, member_name:string) =
        interface TypeResolver.IDjangoType with
            ...
            member x.Type = TypeResolver.DjangoValue
