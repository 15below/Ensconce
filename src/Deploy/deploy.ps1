...
{
	$tools = @(
		[pscustomobject]@{CacheName='Az-Installer';Tool='Az-Installer';Version='2.74.0';ExeName='azure-cli.msi';RunExe=$true;RunArgs="/quiet /norestart";OctoAgent=$true}
		[pscustomobject]@{CacheName='Kube-Linter';Tool='Kube-Linter';Version='0.7.3';ExeName='kube-linter.exe';RunExe=$false;RunArgs="";OctoAgent=$true}
		...
		[pscustomobject]@{CacheName='KubeCtl';Tool='KubeCtl';Version='1.31.8';ExeName='kubectl.exe';RunExe=$false;RunArgs="";OctoAgent=$true}
	)
	...
	$tools | ForEach-Object {
		if($_.OctoAgent -eq $false -or $OctoAgent -eq $true)
		{
			...
			{
				$ToolDownloadCacheFolder = "$DownloadCachePath\$CacheName"
